...
function checksCreateTodosUserAvailability(request, response, next) {
  const { user } = request;
  const { pro: isPaid, todos } = user;

  if (!isPaid && todos.length >= 10) {
    return response
      .status(403)
      .json({ error: "A pro plan is needed for having more than 10 todos." });
  }
...
function findUserById(request, response, next) {
  const { id } = request.params;
  const userQueried = users.find((user) => user.id === id);

  if (userQueried) {
    request.user = userQueried;
    return next();
  }
